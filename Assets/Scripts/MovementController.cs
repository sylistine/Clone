	...
	void Update ()
    {
        if(input.tap)
        {
            ...
            if (Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out hit))
            {
                ...
                {
                    float targetDist = Vector3.Distance(this.transform.position, hit.transform.position);
                    var charData = this.GetComponent<CharacterData>();
                    this.transform.rotation = Quaternion.LookRotation(hit.transform.position - this.transform.position, Vector3.up);
                    if (targetDist < charData.attackRange)
                    {
                        if (charData.attackCooldown == 0)
                        {
                            targetData.current -= charData.attackDamage;
